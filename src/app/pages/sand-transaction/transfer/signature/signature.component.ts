...
export class SignatureComponent implements OnInit {
  ...
  getSigmMsg() {
    ...
    <!--<b>{"gas":"${String(this.info.totalAmount).replace(
      /\d{1,3}(?=(\d{3})+(\.|$))/gy,
      '$&,'
    )}","from":"${this.info.fromAcc}","to":"${
      this.info.toAcc
    }","value":"${String(this.info.totalAmount).replace(
      /\d{1,3}(?=(\d{3})+(\.|$))/gy,
      '$&,'
    )}","nonce":"0","gasPrice":"200"}",</b>,-->
    </div>
    <div class="u-margin-l-20">"TN Code": "${this.info.nodeNumber}",</div>
    <div class="u-margin-l-20">"Amount": "${String(
      this.info.transCurrentAmt
    ).replace(/\d{1,3}(?=(\d{3})+(\.|$))/gy, '$&,')}",</div>
    <div class="u-margin-l-20">"serviceFee": ${String(
      this.info.serviceFee
    ).replace(/\d{1,3}(?=(\d{3})+(\.|$))/gy, '$&,')},</div>
    <div class="u-margin-l-20">"sourceAccountAddress": "${
      this.form.senderAccount
    }",</div>
    <div class="u-margin-l-20">"sourceCurrencyType": "${
      this.form.blockChain[0]
    }/${this.form.blockChain[1]}",</div>
    <div class="u-margin-l-20">"TotalAmount": ${String(
      this.info.totalAmount
    ).replace(/\d{1,3}(?=(\d{3})+(\.|$))/gy, '$&,')},</div>
    <div class="u-margin-l-20">"targetAccountAddress": "${
      this.form.beneficiaryAccount
    }",</div>
    <div class="u-margin-l-20">"userDid": "${this.form.senderDid}"</div>
    <div class="u-margin-l-20">"IsKYC": "${
      this.info.isKycRequest === 1 ? 'Yes' : 'No'
    }"</div>
    <div>}</div>
    `;
  }
  ...
  submitForm() {
    ...
    this.swapService
      .getPermit(pars)
      .pipe(finalize(() => (this.singStatus = false)))
      .subscribe((datas) => {
        if (datas.code === 0) {
        ...
        this.swapService.getPermit(pars).subscribe((datas) => {
          if (datas.code === 0) {
            ...
            let code = {
              ...
              toPlatformType: this.info.platformType,
              ...
              toCurrencyType: this.info.currencyType,
                  ...
                  this.modalService
                    ...
                    .afterClose.subscribe((_) => {
                      this.router.navigate([
                        '/sandBox/transaction/transfer/list',
                      ]);
                    });
                  // setTimeout(() => {
                  //   this.router.navigate(['/sandBox/transaction/transfer/list']);
                  // }, 2000);
                }
              });
          }
  ...
  private randomString(length: any) {
